...
# =============================================
    ...
    with st.expander("📊 Visualização Estática do Grafo (Métricas de Centralidade)", expanded=False):
        ...
        try:
            ...
            st.error(f"Não foi possível calcular {metric_option}: {str(e)}")
            centrality = None
        
        if centrality:
            # Configurações do plot
            plt.figure(figsize=(12, 10), facecolor="#0B0C2A")  # fundo geral da figura
            ax = plt.gca()
            ax.set_facecolor("#0B0C2A")  # fundo dos eixos
            
            # Normalizar valores de centralidade para tamanho e cor
            cent_values = np.array(list(centrality.values()))
            sizes = 50 + 500 * (cent_values - min(cent_values)) / (max(cent_values) - min(cent_values) + 0.01)
            colors = cent_values
            ...
            plt.title(f"{title}\n(Layout: {layout_option})", fontsize=12, color='white')
            plt.colorbar(nodes, label="Valor de Centralidade", shrink=0.8)
            plt.axis('off')
            ...
            - **Layout**: {layout_option} (pode ser alterado acima)
            """)
            
            # Adicionar tabela com top nós
            st.markdown("### 🔝 Top 10 Nós por Centralidade")
            top_10 = sorted(centrality.items(), key=lambda x: x[1], reverse=True)[:10]
            st.dataframe(pd.DataFrame(top_10, columns=["Nó", "Centralidade"]).style.format({"Centralidade": "{:.4f}"}))
        else:
            st.warning("Não foi possível gerar a visualização para esta métrica.")
...
with st.expander("🌐 Visualização Interativa do Grafo", expanded=False):
    ...
    with col1:
        ...
        min_in, max_in = (min(in_degrees.values(), default=0), max(in_degrees.values(), default=1))
        min_out, max_out = (min(out_degrees.values(), default=0), max(out_degrees.values(), default=1))

        in_degree_range = st.slider("Grau de Entrada (in-degree)", min_in, max_in, (2, max_in // 4))
        out_degree_range = st.slider("Grau de Saída (out-degree)", min_out, max_out, (2, max_out // 4))

    with col2:
        show_largest_scc = st.checkbox("Maior SCC (dirigido)", value=False)

    with col3:
        show_largest_wcc = st.checkbox("Maior WCC", value=False)

    # ====== APLICAÇÃO DOS FILTROS ======
    if show_largest_scc and nx.is_directed(G):
        largest_scc = max(nx.strongly_connected_components(G), key=len)
        SG = G.subgraph(largest_scc).copy()
        st.info(f"Subgrafo SCC → {SG.number_of_nodes()} nós, {SG.number_of_edges()} arestas")
    elif show_largest_wcc:
        largest_wcc = max(nx.weakly_connected_components(G), key=len)
        SG = G.subgraph(largest_wcc).copy()
        st.info(f"Subgrafo WCC → {SG.number_of_nodes()} nós, {SG.number_of_edges()} arestas")
    else:
        SG_nodes = [
            n for n in G.nodes()
            if in_degree_range[0] <= in_degrees.get(n, 0) <= in_degree_range[1]
            or out_degree_range[0] <= out_degrees.get(n, 0) <= out_degree_range[1]
        ]
        SG = G.subgraph(SG_nodes).copy()
        st.info(f"Subgrafo filtrado → {SG.number_of_nodes()} nós, {SG.number_of_edges()} arestas")

    # ====== BOTÃO PARA VISUALIZAR GRAFO ======
    if st.button("🔄 Gerar Visualização do Grafo"):
        net = Network(
            notebook=False,
            height="750px",
            width="100%",
        ...
        net.force_atlas_2based(
            gravity=-30, central_gravity=0.005, spring_length=150,
            spring_strength=0.03, damping=0.85, overlap=1,
        )
        ...
        for node in SG.nodes():
            k_in, k_out = in_degrees.get(node, 0), out_degrees.get(node, 0)
            total = k_in + k_out if k_in + k_out > 0 else 1
            balance_ratio = abs(k_in - k_out) / total

            if k_in > k_out:
                hue, tipo = 290 - (70 * balance_ratio), "Receptor"
            elif k_out > k_in:
                hue, tipo = 290 - (280 * balance_ratio), "Emissor"
            else:
                hue, tipo = 290, "Balanceado"

            lightness = 60 - min(20, (total / max(max_in, max_out)) * 20)
            cor = f"hsl({hue}, 70%, {lightness}%)"
            titulo = f"Nó: {node}\nTipo: {tipo}\nGrau entrada: {k_in}, saída: {k_out}"
